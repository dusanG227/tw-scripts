...
(function() {
  ...
  panel.style.cssText = 'position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);z-index:99999;background:#f4e4bc;border:2px solid #7d510f;border-radius:8px;padding:20px;font-family:Verdana,sans-serif;font-size:12px;color:#3e2b0d;width:450px;max-height:90vh;overflow-y:auto;box-shadow:0 5px 30px rgba(0,0,0,0.5);';

  var html = '<div style="text-align:center;margin-bottom:12px;">';
  html += '<h2 style="margin:0;color:#7d510f;font-size:18px;">⚔️ TW Fake Generator</h2>';
  html += '<p style="margin:2px 0 0;font-size:10px;color:#8b7355;">v2.0 — synchronizovaný s fakeScriptMain.js</p>';
  html += '</div>';
  ...
  html += '<div style="font-size:9px;color:#8b7355;">0 = neobmedzené</div></td></tr>';
  ...
  html += '<textarea id="tw-result" rows="3" readonly style="width:100%;padding:5px;border:1px solid #7d510f;border-radius:3px;background:#fff8e7;margin-top:3px;font-family:monospace;font-size:10px;"></textarea>';
  ...
  html += '<p style="margin-top:5px;font-size:10px;color:#8b7355;">💡 Skopíruj a vytvor novú záložku v prehliadači. Spusti ju na stránke Kombinované (overview_villages&mode=combined).</p>';
  html += '</div>';

  // Info
  html += '<div style="margin-top:8px;padding:8px;background:#fff3cd;border:1px solid #ffc107;border-radius:4px;font-size:10px;">';
  html += '<p style="margin:0;">⚔️ <b>Fake limit</b> = % populácie z dediny, ktoré sa pošlú ako fake</p>';
  html += '<p style="margin:2px 0 0;">📊 <b>Max fejkov na cieľ</b> = max útokov na jednu enemy dedinu</p>';
  html += '<p style="margin:2px 0 0;">🏠 <b>Max fejkov z dediny</b> = max útokov z jednej tvojej dediny</p>';
  html += '<p style="margin:2px 0 0;">🎯 <b>Podmienka</b> = fejk sa pošle len ak máš barana, katapult alebo špeha</p>';
  html += '</div>';
  ...
  document.getElementById('tw-generate-btn').onclick = function() {
    ...
    alert('✅ Vygenerované pre ' + coords.length + ' cieľov!\nMax/cieľ: ' + (maxFakesPerTarget || '∞') + ' | Max/dedina: ' + (maxFakesPerVillage || '∞'));
  };
