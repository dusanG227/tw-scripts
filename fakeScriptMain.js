...
// One-bookmarklet flow: útoky sa otvárajú už predvyplnené cez URL
...
(function() {
  ...
  var fakeLimit = Number(config.fakeLimit) || 0.5;
  var openTabs = Number(config.openTabs) || 5;
  var maxFakesPerTarget = Number(config.maxFakesPerTarget) || 0;
  var maxFakesPerVillage = Number(config.maxFakesPerVillage) || 0;
  ...
  if (!isCombined) {
    if (window.location.href.indexOf('screen=place') !== -1) {
      alert('ℹ️ Tento skript spúšťaj iba na Kombinovanej stránke. Útočné taby sa už otvárajú predvyplnené.');
      return;
    }
  ...
  if (!villages.length) {
    alert('⚠️ Nenašli sa žiadne dediny s jednotkami na Kombinovanej stránke.');
    return;
  }

  var attackQueue = buildAttackQueue(villages, targets, fakeLimit);
  log('⚔️ Naplánovaných útokov: ' + attackQueue.length);

  if (!attackQueue.length) {
    alert('⚠️ Nevytvoril sa žiadny útok. Skontroluj limity, jednotky a arrival okno.');
    return;
  }
  ...
  function selectUnitsForFake(availableUnits, fakeLimitPct) {
    ...
    // Random spy count 1-4 (or max available)
    var wantedSpy = randInt(1, 4);
    ...
    // Global fill: use ALL available non-siege units to reach budget
    if (usedPop < maxPop) {
      var allFillers = ['light', 'spear', 'axe', 'archer', 'marcher', 'heavy', 'sword'];
      for (var s2 = allFillers.length - 1; s2 > 0; s2--) {
        var j2 = Math.floor(Math.random() * (s2 + 1));
        var tmp2 = allFillers[s2]; allFillers[s2] = allFillers[j2]; allFillers[j2] = tmp2;
      }
      for (var g = 0; g < allFillers.length && usedPop < maxPop; g++) {
        var gn = allFillers[g];
        var gHave = (availableUnits[gn] || 0) - (selected[gn] || 0);
        if (gHave <= 0) continue;
        var gPop = unitPop[gn] || 1;
        var gTake = Math.min(Math.floor((maxPop - usedPop) / gPop), gHave);
        if (gTake > 0) {
          selected[gn] = (selected[gn] || 0) + gTake;
          usedPop += gTake * gPop;
        }
      }
    }
    ...
    while (queue.length < maxTotal && safety-- > 0) {
      ...
      for (var v = 0; v < preparedVillages.length; v++) {
        ...
        while (tries < targetList.length) {
          ...
          if (!isInArrivalWindow(pv.village, target, pv.units)) continue;
  ...
  function showControlPanel(queue) {
    ...
    panel.style.cssText = 'position:fixed;top:10px;right:10px;z-index:99999;background:#f4e4bc;border:2px solid #7d510f;border-radius:8px;padding:15px;font-family:Verdana,sans-serif;font-size:11px;color:#3e2b0d;width:390px;max-height:80vh;overflow-y:auto;box-shadow:0 5px 30px rgba(0,0,0,0.5);';
    ...
    if (arrivalStart || arrivalEnd) {
      h += '<br/>Arrival: ' + (arrivalStart ? arrivalStart.toLocaleString('sk-SK') : '?') + ' - ' + (arrivalEnd ? arrivalEnd.toLocaleString('sk-SK') : '?');
    }
    h += '</div>';
